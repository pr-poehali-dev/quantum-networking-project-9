  ...
  return (
    <div className="grid grid-cols-1 gap-1.5 md:grid-cols-2 lg:grid-cols-8">
      ...
      >
        ...
        <div className="flex flex-col p-3 lg:flex-row">
          ...
          <ul className="text-gray-300 grid gap-2 text-xs lg:w-[70%] font-open-sans-custom">
            {[
              "До 6 участников в семье",
              "Лимиты и цели для каждого участника",
              "Детальная аналитика и диаграммы расходов",
              "Уведомления о перерасходе в реальном времени",
            ].map((f, i) => (
              <li key={i} className="flex items-center gap-2">
                <Check className="w-[1.05rem] h-[1.05rem] text-white flex-shrink-0" strokeWidth={3} />
      ...
      <PricingCard
        ...
        priceLabel="349 ₽"
        priceSuffix="/мес"
        features={["До 4 участников", "Роли и права доступа", "Уведомления о перерасходе"]}
        className="lg:col-span-4"
        cta="Подключить"
      />

      <PricingCard
        titleBadge="РАСШИРЕННЫЙ"
